...
const { processWorkbook, processPostponedFile } = require('./processorCore');

// State tracking (kept module-local as in original)
let globalCustomerID = '';
let globalCustomerName = '';
let hasCustomerError = false;
let isShippingGroup = false;

// Global set for duplicate checks (as in original)
...
async function handleDocumentMessage(msg) {
    ...
    if (chat.isGroup) {
        console.log(`📥 Received message from group: ${COLORS.CYAN}${chat.name}${COLORS.RESET} | type: ${COLORS.YELLOW}${msg.type}${COLORS.RESET} | body: ${COLORS.GREEN}${msg.body}${COLORS.RESET}`);
    } else {
    ...
    let media;
    try {
        // Attempt to reload message to ensure full data (fixes some 'addAnnotations' errors)
        try {
            await msg.reload();
        } catch (reloadErr) {
            console.warn('⚠️ msg.reload() failed, trying download anyway:', reloadErr);
        }
    ...
    if (rowProblems.size === 0 && dateError == "") {
        ...
        for (let i = 0; i < allRows.length; i++) {
            ...
            if (!isLoloCase || isHardCase) {
                ...
                // Release Paper Logic:
                // Normal Case: All FCL -> Print All
                // Normal Case: FCL + EMPTY + !LOLO -> Print Empty Only
                // Hardcase: Print All

                // Mixed FCL/EMPTY Logic moved to top (Priority 1)
        ...
        let jobNo = JobNumberService.getOrCreateJobNumber(globalCustomerID, globalCustomerName);
